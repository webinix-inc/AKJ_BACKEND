...
class PerformanceMonitor {
  ...
  calculateHttpMetrics() {
    ...
    if (this.requestTimes.length > 0) {
      ...
      this.metrics.http.averageResponseTime = Math.round(
    ...
    const totalResponses = Object.values(this.metrics.http.statusCodes).reduce(
      (a, b) => a + b,
      0
    );
    if (totalResponses > 0) {
  ...
  calculateRates() {
    ...
    if (timeDiff > 0) {
      this.metrics.http.requestsPerSecond = Math.round(
        requestDiff / Math.min(timeDiff, 30)
      );
    }
  ...
  checkAlerts() {
    ...
    alerts.slowResponseTime =
      this.metrics.http.averageResponseTime > this.thresholds.responseTime;

    // Error Rate Alert
    alerts.highErrorRate =
      this.metrics.http.errorRate > this.thresholds.errorRate;

    // Database Connections Alert
    alerts.databaseConnectionsHigh =
      this.metrics.database.connections.current > this.thresholds.dbConnections;
  ...
  logPerformanceMetrics() {
    ...
    // logger.system(
    //   `Performance: CPU ${perf.system.cpu}, Memory ${perf.system.memory}, RPS ${perf.http.rps}, Response ${perf.http.avgResponse}, Users ${perf.socket.connected}`
    // );
  }
  ...
  getCapacityAssessment() {
    ...
    const responseScore = Math.max(
      0,
      100 - this.metrics.http.averageResponseTime / 10
    );
    const errorScore = Math.max(0, 100 - this.metrics.http.errorRate * 5);

    const overallScore = Math.round(
      (cpuScore + memoryScore + responseScore + errorScore) / 4
    );
  ...
  getRecommendations() {
    ...
    if (this.metrics.alerts.highMemoryUsage) {
      recommendations.push(
        "Monitor memory leaks and consider increasing server memory"
      );
    }
