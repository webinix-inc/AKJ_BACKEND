...
exports.createCoupon = async (req, res) => {
    try {
        const {
            offerName, couponCode, couponType, assignedUserIds,
            courseSelectionType, assignedCourseIds, usageLimit,
            isUnlimited, usagePerStudent, visibility, discountType,
            discountAmount, startDate, startTime, endDate, endTime,
            isLifetime, minimumOrderValue
        } = req.body;
        ...
        if (couponType === 'Private' && (!assignedUserIds || assignedUserIds.length === 0)) {
            return res.status(400).json({ message: "Private coupons require assigned users" });
        }

        if (courseSelectionType === 'Specific' && (!assignedCourseIds || assignedCourseIds.length === 0)) {
            return res.status(400).json({ message: "Specific course selection requires assigned courses" });
        }
        ...
        const coupon = new Coupon({
            ...
            courseSelectionType,
            assignedCourses: courseSelectionType === 'Specific' ? assignedCourseIds : [],
...
exports.updateCoupon = async (req, res) => {
    try {
        const {
            offerName, couponType, assignedUserIds,
            courseSelectionType, assignedCourseIds, usageLimit,
            isUnlimited, usagePerStudent, visibility, discountType,
            discountAmount, startDate, startTime, endDate, endTime,
            isLifetime, minimumOrderValue
        ...
        coupon.courseSelectionType = courseSelectionType || coupon.courseSelectionType;
        ...
        coupon.visibility = visibility !== undefined ? visibility : coupon.visibility;
...
exports.validateCoupon = async (req) => {
    try {
        ...
        if (
            ...
            return { success: false, message: "Coupon has expired or not yet active" };
        }
        ...
        if (coupon.couponType === "Private" && !coupon.assignedUsers.includes(userId)) {
        ...
        if (
            ...
            return { success: false, message: "Coupon not applicable for this course" };
        }
        ...
        return { success: false, message: "Error validating coupon", error: error.message };
    }
};
